...
import { PlanContent } from "@/components/document/content/PlanContent";
import { PlanForm } from "@/components/forms/PlanForm";
import type { ApprovalHistoryEntry, Attachment, PlanDocument, Standard } from "@/lib/types";
import type { PlanFormValues } from "@/lib/schemas/plan";

const FORM_TITLE = "개발계획서 (F702-1)";
...
export default function PlanDocumentPage() {
  ...
  const [doc, setDoc] = useState<(PlanDocument & { id: string }) | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    if (!isNew) {
      ...
      const [d, h, a] = await Promise.all([
        getDocumentById<PlanDocument>("PLAN", docId),
        listApprovalHistory("PLAN", docId),
        listAttachmentsByDocument("PLAN", docId),
      ]);
      setDoc(d);
      setHistory(h);
      setAttachments(a);
  ...
  async function handleSubmit(values: PlanFormValues) {
    ...
    if (isNew) {
      ...
      const newId = await createDocument<PlanDocument>("PLAN", projectId, projectCode, user.uid, {
        ...values,
        attachmentIds: [],
      });
      router.push(`/projects/${projectId}/plan/${newId}`);
      return;
    }
    await updateDocumentBody("PLAN", docId, values);
    await load();
  }
    ...
    return (
      <div>
        ...
        <PrintLayout formTitle={FORM_TITLE} projectName={projectName} header={doc.header}>
          <PlanContent doc={doc} standardNames={standardNames} variant="print" />
        </PrintLayout>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {viewMode === "detail" && doc ? (
        <PlanContent doc={doc} standardNames={standardNames} variant="detail" />
      ) : (
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <PlanForm defaultValues={doc ?? undefined} readOnly={readOnly} onSubmit={handleSubmit} />
          </div>
          {!isNew && doc && (
            <div className="space-y-4">
              <ApprovalActions docType="PLAN" docId={docId} header={doc.header} onChanged={load} />
              <AttachmentUploader
                documentType="PLAN"
                documentId={docId}
